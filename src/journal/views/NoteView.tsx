...
export const NoteView = (): JSX.Element => {
    ...
    const { body, title, date, onInputChange, formState } = useForm<Note>(activeNote);
    ...
    const onFileInputChange = (event: ChangeEvent<HTMLInputElement>): void => {
        ...
        dispatch(startUploadingFiles(imageFiles));
    };
    ...
    return (
        <Grid container direction='row' justifyContent='space-between' alignItems='center' sx={{ mb: 1 }} className='animate__animated animate__fadeIn animate__faster'>
            ...
            <Grid container >
                <TextField onChange={onInputChange} value={title} name='title' type='text' variant='filled' fullWidth placeholder='Ingrese un título' label='Título' sx={{ border: 'none', mb: 1 }} />
                <TextField onChange={onInputChange} value={body} name='body' type='text' variant='filled' fullWidth multiline placeholder='¿Qué sucedió en el día de hoy?' minRows={5} />
            </Grid>
